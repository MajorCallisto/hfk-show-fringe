...
const SplatMaterial = /* @__PURE__ */ shaderMaterial(
  ...
  /*glsl*/ `
    ...
    void main () {
      ...
      // Add a consistent offset per splat
      vec3 randomOffset = vec3(
        fract(sin(dot(vec2(texPos), vec2(12.9898,78.233))) * 43758.5453),
        fract(sin(dot(vec2(texPos), vec2(93.9898,67.345))) * 15731.1234),
        fract(sin(dot(vec2(texPos), vec2(52.9898,24.345))) * 24521.2311)
      ) * 2.0 - 1.0; // Range [-1, 1]

      // vec3 displacedCenter = center.xyz + randomOffset * 3.0; // Customize displacement strength
      // vec3 animatedCenter = mix(displacedCenter, center.xyz, clamp(uTime / 0.25, 0.0, 1.0));
      // center = vec4(animatedCenter, 1.0);
      ...
      vec2 v1 = min(sqrt(2.0 * lambda1), 1024.0) * radiusScale * diagonalVector;
      vec2 v2 = min(sqrt(2.0 * lambda2), 1024.0) * radiusScale * vec2(diagonalVector.y, -diagonalVector.x);
